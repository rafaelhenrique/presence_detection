...
class AWSPublisher(object):
    ...
    def send(self, data, debug=False):
        ...
        # client.tls_set(self.root_cert_file, certfile=self.certificate_file,
        ...
        try:
            ...
            # client.connect(self.address, self.port, self.keepalive)
            # client.loop_start()
            # status = client.publish(self.topic, payload=json.dumps(payload),
            #                         qos=0)
...
if __name__ == '__main__':
    ...
    certificate_file = os.path.join(keys_path, 'light_sensor.cert.pem')
    private_key_file = os.path.join(keys_path, 'light_sensor.private.key')
    hostname = socket.gethostname()
    publisher = AWSPublisher(log_file='publisher.log',
                             topic='$aws/things/light_sensor/shadow/update',
